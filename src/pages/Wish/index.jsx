...
const Wish = () => {
    ...
    return (
        <section id="wish" className="container h-100">
            ...
            <p className="text-center fs-5 border-bottom border-3 border-black pb-2">Kirim harapan dan doa untuk kedua mempelai</p>
            ...
            <div className="d-flex flex-column justify-content-center h-75 px-4">
                <form name="submit-to-google-sheet container w-100" id="form" className="px-4 form-wrapper row py-1" method="post">
                    <div className="col-lg-6 mb-4">
                        <input className="shadow rounded-1 border-0 form-control" name="nama" type="text" placeholder="Nama" onChange={ handleNameChange } value={ formData.name }/>
                    </div>
                    <div className="col-lg-6 mb-4">
                        <input className="shadow rounded-1 border-0 form-control" name="no" type="number" placeholder="Nomor ponsel (opsional)" />
                    </div>
                ...
                <div className="pt-4 w-100 d-flex flex-column align-items-end position-relative px-4">
                    <a href='blank' type="submit" className={` ${style['btn-details']} py-2 px-3 text-black rounded-2 border-0 text-black fw-semibold ${isLoading ? 'disabled' : '' }`} aria-disabled="true" onClick={ handleSubmit }>
                        { isLoading ? <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> : <span className="fw-bold">Kirim</span> }
                    </a>
